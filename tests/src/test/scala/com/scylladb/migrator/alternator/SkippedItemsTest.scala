...
class SkippedItemsTest extends MigratorSuiteWithDynamoDBLocal {
  ...
  withTable("TtlTable").test(
    "Expired items should be filtered out from the source table (PAY_PER_REQUEST)"
  ) { tableName =>
    runTtlTest(tableName, "dynamodb-to-alternator-ttl.yaml")
  }

  withTable("TtlTable").test(
    "Expired items should be filtered out from the source table (PROVISIONED)"
  ) { tableName =>
    runTtlTest(tableName, "dynamodb-to-alternator-ttl-provisioned.yaml")
  }

  private def runTtlTest(tableName: String, configFile: String): Unit = {
