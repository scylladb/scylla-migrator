...
class DynamoDBS3ExportWriterTest extends munit.FunSuite {
  ...
  private val tempDir: FunFixture[File] = FunFixture(
    setup =
      _ => new File(System.getProperty("java.io.tmpdir"), s"s3export-test-${System.nanoTime()}"),
    teardown = dir => if (dir.exists()) com.scylladb.migrator.TestFileUtils.deleteRecursive(dir)
  )
  ...
  tempDir.test("writer output can be parsed by reader's item decoder (round-trip)") { outputDir =>
    ...
    for (f <- dataFiles) {
      ...
      for (line <- lines) {
        ...
        assert(
          decoded.isRight,
          s"Reader failed to parse writer output: ${decoded.left.getOrElse("")}"
        )
    ...
    assert(
      readerSummary.isRight,
      s"Reader failed to parse manifest-summary: ${readerSummary.left.getOrElse("")}"
    )
  }
  ...
  tempDir.test("writeRDD with tableDescription includes key schema in manifest summary") {
    outputDir =>
      val rdd = makeRDD(5, 1)
      val target = TargetSettings.DynamoDBS3Export(outputDir.getAbsolutePath)
      ...
      val keySchema =
        cursor.downField("tableKeySchema").as[List[io.circe.Json]].fold(throw _, identity)
      ...
      val attrDefs = cursor
        .downField("tableAttributeDefinitions")
