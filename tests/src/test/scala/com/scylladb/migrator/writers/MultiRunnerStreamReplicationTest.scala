...
class MultiRunnerStreamReplicationTest extends StreamReplicationTestFixture {
  ...
  /** Create a TestStreamPoller that advertises the given shards and delivers items exactly once per
    ...
    *
    * @param deliveryCounter
    *   optional shared counter incremented for each record delivered, used to detect duplicate
    *   processing across multiple pollers sharing the same shard.
    */
  private def makeTestPoller(
    shards: Seq[String],
    itemsPerShard: Int,
    deliveryCounter: Option[AtomicInteger] = None
  ): TestStreamPoller = {
    ...
    poller.getRecordsFn.set { (_, iterator, _) =>
      ...
      if (!flag.getAndSet(true)) {
        ...
        deliveryCounter.foreach(_.addAndGet(records.size))
  ...
  test("parallel shard splitting: two runners split shared shards via lease contention") {
    val sourceSettings = makeSourceSettings()
    val tableDesc = getTargetTableDesc()

    // Both runners see all 4 shards and contend through the lease system.
    val allShards = Seq("shard-A", "shard-B", "shard-C", "shard-D")
    val totalDelivered = new AtomicInteger(0)
    val poller1 = makeTestPoller(allShards, itemsPerShard = 3, Some(totalDelivered))
    val poller2 = makeTestPoller(allShards, itemsPerShard = 3, Some(totalDelivered))
    ...
    // Verify each shard's items were delivered exactly once (12 total, not 24).
    // If both runners processed the same shard, totalDelivered would exceed 12.
    assertEquals(
      totalDelivered.get(),
      12,
      s"Expected exactly 12 records delivered (each shard by one runner only), " +
        s"got ${totalDelivered.get()} — indicates double-claiming"
    )
  ...
  test("shared shard: no duplicate processing when two runners see the same shard") {
    ...
    // Shared counter to detect if both pollers deliver records (which would
    // mean the lease system failed to prevent double-claiming).
    val totalDelivered = new AtomicInteger(0)
    val poller1 = makeTestPoller(Seq("shared-shard"), itemsPerShard = 5, Some(totalDelivered))
    val poller2 = makeTestPoller(Seq("shared-shard"), itemsPerShard = 5, Some(totalDelivered))
    ...
    // Verify that only one poller delivered records (5, not 10).
    // If both pollers delivered, the lease system failed to prevent double-claiming.
    assertEquals(
      totalDelivered.get(),
      5,
      s"Expected exactly 5 records delivered (one poller only), " +
        s"got ${totalDelivered.get()} — indicates duplicate processing"
    )
  ...
  test("sequential failover: runner 2 resumes from checkpoint after runner 1 stops") {
    ...
    // Manually expire all leases in the checkpoint table so runner 2 can claim
    // shards immediately, instead of waiting for time-based expiry (which is flaky under CI load).
    val ckptItems = sourceDDb()
      .scan(ScanRequest.builder().tableName(realCheckpointTable).build())
      .items()
    ckptItems.forEach { item =>
      val key = item.get("leaseKey")
      if (key != null) {
        sourceDDb().updateItem(
          software.amazon.awssdk.services.dynamodb.model.UpdateItemRequest
            .builder()
            .tableName(realCheckpointTable)
            .key(Map("leaseKey" -> key).asJava)
            .updateExpression("SET leaseExpiryEpochMs = :zero")
            .expressionAttributeValues(
              Map(":zero" -> AttributeValue.fromN("0")).asJava
            )
            .build()
        )
      }
