...
class LeaseTransferTest extends MigratorSuiteWithDynamoDBLocal {
  ...
  test("requestLeaseTransfer sets transfer field on an existing shard") {
    ...
    assert(claimed.isInstanceOf[ClaimResult.Claimed])
  ...
  test("renewLeaseAndCheckpoint detects transfer request and releases lease") {
    ...
    // Worker 1 renews — should see the transfer request
    val result = DefaultCheckpointManager.renewLeaseAndCheckpoint(
      sourceDDb(),
      checkpointTable,
      "shard-1",
      worker1,
    ...
    assertEquals(
      ...
      "renewLeaseAndCheckpoint should return TransferRequested when transfer is requested"
    )

    // Lease is NOT released automatically — the caller must release after checkpointing
    DefaultCheckpointManager.releaseLease(sourceDDb(), checkpointTable, "shard-1", worker1)
