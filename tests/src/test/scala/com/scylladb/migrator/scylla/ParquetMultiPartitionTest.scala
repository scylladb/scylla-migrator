...
class ParquetMultiPartitionTest extends ParquetMigratorSuite {
  ...
  /** Run migration with custom Spark configuration to force file splitting. This sets
    * spark.sql.files.maxPartitionBytes to a very small value (64KB) to ensure even small files get
  ...
  withTableAndSavepoints("multipartitiontest2", "multipartition2", "parquet-multipartition-test2")
    .test(
