...
class ParquetToScyllaBasicMigrationTest extends ParquetMigratorSuite {
  ...
  FunFixture.map2(withTable("BasicTest"), withParquetDir("root")).test("Basic migration from Parquet to ScyllaDB") { case (tableName, parquetRoot) =>
    writeParquetTestFile(
      parquetRoot.resolve("basic.parquet"),
