...
/** Manage DynamoDB-based migrations by tracking the migrated scan segments.
  */
class DynamoDbSavepointsManager(
...
object DynamoDbSavepointsManager {
  ...
  /** @return
    *   The scan segments processed in partition `partitionId` of `rdd`.
    */
  private def scanSegments(
    rdd: RDD[(Text, DynamoDBItemWritable)],
    partitionId: Int
  ): Try[java.util.List[Integer]] =
  ...
  /** @return
    *   The `DynamoDBSplit` wrapped by the `partition`. Fails if the `partition` is not a
    *   `HadoopPartition` containing a `DynamoDBSplit`.
    */
