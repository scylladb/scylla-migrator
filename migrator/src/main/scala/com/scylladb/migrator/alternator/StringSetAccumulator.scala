...
/** Accumulator for tracking processed Parquet file paths during migration.
  *
  * This accumulator collects the set of Parquet file paths that have been processed as part of a
  * migration job. It is useful for monitoring progress, avoiding duplicate processing, and
  * debugging migration workflows. The accumulator is thread-safe and can be used in distributed
  * Spark jobs.
  *
  * @param initialValue
  *   The initial set of processed file paths (usually empty).
  */
