  ...
  def recordToItem(
    ...
    eventName match {
      case OperationType.INSERT | OperationType.MODIFY =>
        ...
        // Add keys first, then newImage. newImage is authoritative and should
        // take precedence for key attributes (values should be identical in
        // practice, but this ordering is safer if they ever differ).
