...
object RowComparisonFailure {
  ...
  private[migrator] def areDifferent(
    ...
    (leftValue, rightValue) match {
      ...
      // Special handling for NaN and Infinity: treat NaN == NaN and +Inf == +Inf, -Inf == -Inf
      case (Some(l: Float), Some(r: Float)) =>
        if (l.isNaN && r.isNaN) false
        else if (l.isInfinity && r.isInfinity) l != r // +Inf != -Inf
        else !DoubleMath.fuzzyEquals(l, r, floatingPointTolerance)
