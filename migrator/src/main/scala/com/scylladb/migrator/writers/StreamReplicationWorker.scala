...
private[writers] class StreamReplicationWorker(
  ...
  /** Shards whose leases were lost during background renewal. The renewal thread enqueues shard IDs
    * here; the scheduler thread drains the queue at the start of each poll cycle. This avoids
    * cross-thread mutation of the `shardSequenceNumbers` var.
    */
  private val lostLeaseShards = new ConcurrentLinkedQueue[String]()

  /** Called by the lease renewal thread when a renewal returns [[LeaseRenewResult.Lost]]. */
  private[writers] def reportLostLease(shardId: String): Unit =
    lostLeaseShards.add(shardId)
  ...
  /** Tracks (cycle count absent, first-absent timestamp in millis) for closed shards. */
  private var closedShardAbsentCycles = Map.empty[String, (Int, Long)]
  ...
  // Compute operational intervals relative to batchIntervalSeconds so behavior
  // stays consistent regardless of poll frequency.
  private val shardCleanupThresholdCycles = math.max(1, 300 / batchIntervalSeconds) // ~5 minutes
  private val shardCleanupMinAbsentMs = 30L * 60L * 1000L // 30 minutes
  private val shardListRefreshInterval = math.max(1, 30 / batchIntervalSeconds) // ~30 seconds
  private val minShardRefreshIntervalMs = 5000L
  private val metricsPublishIntervalCycles = math.max(1, 60 / batchIntervalSeconds) // ~60 seconds
  ...
  def pollAndProcess(): Unit = {
    ...
    try {
      ...
      // Drain shards whose leases were lost by the background renewal thread.
      var lostShard = lostLeaseShards.poll()
      while (lostShard != null) {
        log.warn(s"Removing shard $lostShard from tracking (lease lost during background renewal)")
        shardIterators.remove(lostShard)
        shardSequenceNumbers = shardSequenceNumbers - lostShard
        metrics.checkpointFailures.incrementAndGet()
        lostShard = lostLeaseShards.poll()
      }
      ...
      val (allItems, itemShardInfos, closedShards) = processPollResults(pollResults)
      ...
      if (allItems.nonEmpty)
        try
          ...
          case e: BatchWriteExhaustedException =>
            ...
            // Partially roll back: compute per-shard sequence numbers for the
            // items that were successfully flushed, so only the failed sub-batch
            // and beyond are re-processed on the next poll cycle.
            revertToPartialProgress(prevSeqNums, itemShardInfos, e.successfullyFlushedCount)
          case e: Exception =>
            writeSucceeded = false
            metrics.writeFailures.incrementAndGet()
            log.error("Failed to write batch to target, skipping checkpoint to avoid data loss", e)
            // Full revert: unknown failure point, re-process everything.
      ...
      // - Write failure: increment error counter.
      // - Successful non-empty cycle: reset immediately.
      // - Successful empty cycle: decay by 1 to avoid stale error counts from
      //   persisting indefinitely, while still allowing sustained failures to
      //   accumulate even when interspersed with occasional empty polls.
      if (!writeSucceeded) {
        consecutiveErrors += 1
      ...
      } else if (allItems.nonEmpty && consecutiveErrors > 0) {
        log.info(s"Recovered after $consecutiveErrors consecutive errors")
        consecutiveErrors = 0
      } else if (allItems.isEmpty && consecutiveErrors > 0) {
    ...
    if (!shardIterators.isEmpty) {
      ...
      try Await.result(combined, Duration(pollFutureTimeoutSeconds, TimeUnit.SECONDS))
        ...
        case e: Exception =>
          ...
          // Invalidate iterators for timed-out shards so they are re-acquired
          // on the next poll cycle. The in-flight futures may still complete in
          // the background but their results will be discarded; re-acquiring
          // the iterator ensures those records are re-fetched.
          for (shardId <- timedOutShardIds)
  ...
  /** Per-item shard and sequence number tracking for partial rollback on write failure. Each entry
    * corresponds to an item in the `allItems` list at the same index.
    */
  private case class ItemShardInfo(shardId: String, seqNum: String)

  /** Hard cap on items buffered per poll cycle to prevent OOM on the driver. With many shards and
    * high throughput, unbounded accumulation can exhaust memory. Items beyond this limit are
    * dropped and will be re-fetched on subsequent cycles (their sequence numbers are not advanced).
    */
  private val maxItemsPerCycle = 100000

  private def processPollResults(
    pollResults: Seq[(String, Seq[Record], Option[String])]
  ): (Seq[DynamoItem], Seq[ItemShardInfo], Set[String]) = {
    val totalRecords = pollResults.map(_._2.size).sum
    val allItems =
      new scala.collection.mutable.ArrayBuffer[DynamoItem](math.min(totalRecords, maxItemsPerCycle))
    val itemShardInfos =
      new scala.collection.mutable.ArrayBuffer[ItemShardInfo](
        math.min(totalRecords, maxItemsPerCycle)
      )
    ...
    for ((shardId, records, nextIter) <- pollResults) {
      for (record <- records if !itemCapReached) {
        ...
        poller
          ...
          .foreach { item =>
            if (allItems.size >= maxItemsPerCycle) {
              if (!itemCapReached) {
                log.warn(
                  s"Reached per-cycle item cap ($maxItemsPerCycle). " +
                    s"Remaining records will be re-fetched on subsequent cycles. " +
                    s"Consider reducing maxRecordsPerPoll or the number of shards."
                )
                itemCapReached = true
              }
            } else if (seqNum != null) {
              allItems += item
              itemShardInfos += ItemShardInfo(shardId, seqNum)
              updatedSeqNums(shardId) = seqNum
            } else
              log.warn(
                s"Skipping item from shard $shardId with null sequence number " +
                  "(cannot track for partial rollback)"
              )
          }
      }
      nextIter match {
    ...
    (allItems.toSeq, itemShardInfos.toSeq, closedShards.toSet)
  }
    ...
    for {
      ...
      if (!checkpointMgr.isParentDrained(checkpointClient, checkpointTableName, parentId)) {
        ...
        checkpointMgr.tryClaimShard(
          ...
          case ClaimResult.Claimed(checkpointOpt) =>
            log.info(
          ...
          case ClaimResult.OwnedByAnother =>
            () // Another worker owns this shard
          case ClaimResult.TransientError =>
            log.debug(s"Transient error claiming shard ${shard.shardId()}, will retry next cycle")
        }
    }
  }
  ...
  private def cleanupClosedShards(
    ...
    closedShardAbsentCycles = closedShardAbsentCycles.map {
      case (sid, _) if activeShardIds.contains(sid) => sid -> (0, now)
      case (sid, (count, firstAbsentMs))            => sid -> (count + 1, firstAbsentMs)
    }
    for (shardId <- closedShards if !closedShardAbsentCycles.contains(shardId))
      closedShardAbsentCycles = closedShardAbsentCycles + (shardId -> (0, now))
    val toCleanup = closedShardAbsentCycles.collect {
      case (sid, (count, firstAbsentMs))
          if count >= shardCleanupThresholdCycles &&
            (now - firstAbsentMs) >= shardCleanupMinAbsentMs =>
        sid
    }
  ...
  /** Roll back per-shard sequence numbers to reflect only successfully flushed items.
    *
    * For each shard, the sequence number is set to the last value seen in the first `flushedCount`
    * items. Shards with no items in that range revert to `prevSeqNums`.
    */
  private def revertToPartialProgress(
    prevSeqNums: Map[String, String],
    itemShardInfos: Seq[ItemShardInfo],
    flushedCount: Int
  ): Unit =
    if (flushedCount <= 0)
      shardSequenceNumbers = prevSeqNums
      ...
      val partialSeqNums =
        scala.collection.mutable.Map[String, String](prevSeqNums.toSeq: _*)
      itemShardInfos.take(flushedCount).foreach { info =>
        partialSeqNums(info.shardId) = info.seqNum
      }
      shardSequenceNumbers = partialSeqNums.toMap
    }
      ...
      val result =
        try
          ...
          case e: Exception =>
            ...
            metrics.checkpointFailures.incrementAndGet()
            LeaseRenewResult.Held // keep tracking the shard; the next cycle will retry
        }
      result match {
        case LeaseRenewResult.Held => ()
        case LeaseRenewResult.Lost =>
          metrics.checkpointFailures.incrementAndGet()
        ...
        case LeaseRenewResult.TransferRequested =>
          // Checkpoint was saved; now safe to release the lease for transfer.
          log.info(s"Releasing lease for shard $shardId after checkpoint (transfer requested)")
          checkpointMgr.releaseLease(checkpointClient, checkpointTableName, shardId, workerId)
          shardIterators.remove(shardId)
          shardSequenceNumbers = shardSequenceNumbers - shardId
      }
    }
  ...
  private def markClosedShards(
    ...
    for (shardId <- closedShards)
      try {
        ...
        val result = checkpointMgr.renewLeaseAndCheckpoint(
          checkpointClient,
          checkpointTableName,
          shardId,
          workerId,
        ...
        result match {
          ...
          case LeaseRenewResult.TransferRequested =>
            log.info(s"Releasing lease for closed shard $shardId (transfer requested)")
            checkpointMgr.releaseLease(checkpointClient, checkpointTableName, shardId, workerId)
        }
      } catch {
        case e: Exception =>
        ...
        // Always clean up sequence number tracking for closed shards. The shard has
        // already been removed from shardIterators (won't be polled again). If we
        // don't clean up here on failure, the stale entry could cause duplicate
        // writes if the shard is re-claimed from an old checkpoint.
        shardSequenceNumbers = shardSequenceNumbers - shardId
