...
/** Result of a shard claim attempt. */
sealed trait ClaimResult
object ClaimResult {

  /** Successfully claimed the shard; contains the optional checkpoint sequence number. */
  case class Claimed(checkpoint: Option[String]) extends ClaimResult

  /** Another worker legitimately owns this shard (conditional check failed). */
  case object OwnedByAnother extends ClaimResult

  /** A transient DynamoDB error prevented the claim; the shard should be retried next cycle. */
  case object TransientError extends ClaimResult
}

/** Result of a lease renewal attempt. */
sealed trait LeaseRenewResult

object LeaseRenewResult {

  /** Lease is still held by this worker. */
  case object Held extends LeaseRenewResult

  /** Lease was lost (stolen by another worker or conditional check failed). */
  case object Lost extends LeaseRenewResult

  /** Lease is still held, but another worker has requested a transfer. The caller should release
    * the lease at a safe point (e.g., after checkpointing).
    */
  case object TransferRequested extends LeaseRenewResult
}
...
object DefaultCheckpointManager extends CheckpointManager {
    ...
    try {
      ...
      val checkpoint =
        if (attrs.containsKey(checkpointColumn))
          ...
          None
      ClaimResult.Claimed(checkpoint)
    } catch {
      case _: ConditionalCheckFailedException => ClaimResult.OwnedByAnother
  ...
  override def renewLeaseAndCheckpoint(
    ...
    try {
      ...
      if (transferRequested) {
        val transferTo = attrs.get(leaseTransferToColumn).s()
        log.info(
          s"Lease transfer requested for shard $shardId to $transferTo"
        )
        LeaseRenewResult.TransferRequested
      } else
        LeaseRenewResult.Held
    } catch {
      case _: ConditionalCheckFailedException => LeaseRenewResult.Lost
      case e: Exception =>
  ...
  override def requestLeaseTransfer(
    ...
    try {
      ...
      case e: DynamoDbException
          if e.awsErrorDetails() != null &&
            retryableErrorCodes.contains(e.awsErrorDetails().errorCode()) =>
        log.warn(s"Transient error requesting lease transfer for shard $shardId", e)
        false
      case e: Exception =>
        log.error(s"Non-transient error requesting lease transfer for shard $shardId", e)
        throw e
    }
  ...
  override def releaseLease(
    ...
    try
      ...
      case e: Exception =>
        log.error(s"Failed to release lease for shard $shardId after retries", e)
    }
