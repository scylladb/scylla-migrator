...
object DynamoDBS3Export {
  ...
  def writeRDD(
    ...
    try {
            ...
            if (!buffered.hasNext) {
              ...
              try
                buffered.foreach { case (_, itemWritable) =>
                  require(itemWritable.getItem != null, "DynamoDBItemWritable has null item map")
      ...
      try confBroadcast.destroy()
      catch {
        case NonFatal(e) => log.warn("Failed to destroy broadcast variable", e)
      }
  }
