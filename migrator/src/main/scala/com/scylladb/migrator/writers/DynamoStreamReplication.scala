...
object DynamoStreamReplication {
  ...
  private def shortHash(input: String): String = {
    ...
    digest.digest(input.getBytes("UTF-8")).take(8).map(b => f"${b & 0xff}%02x").mkString
  }
  ...
  class StreamHandle(
    ...
    def stop(): Unit = {
      ...
      // Wait for the scheduler to finish its current cycle so it can
      // checkpoint any already-written items before we release leases.
      // The polling pool must stay alive during this window so that
      // in-flight poll futures can complete normally rather than being
      // interrupted (which would discard already-fetched records).
      if (!executors.scheduler.awaitTermination(30, TimeUnit.SECONDS))
        executors.scheduler.shutdownNow()
      // Now that the scheduler has stopped, shut down the polling pool.
      ...
      if (!executors.pollingPool.awaitTermination(10, TimeUnit.SECONDS))
        executors.pollingPool.shutdownNow()
      // Release leases after the scheduler has terminated so that
      // in-flight pollAndProcess cycles can checkpoint before the lease
      // is freed. This prevents duplicate writes on restart.
  ...
  def startStreaming(
    ...
    // Collect credential providers that need closing (STS assume-role providers have
    // background refresh threads and HTTP connection pools).
    val closeableCredentials: Seq[AutoCloseable] =
      Seq(src.finalCredentials, target.finalCredentials).flatten.collect { case c: AutoCloseable =>
        c
      }

    // All clients are created; wrap the rest in try/catch to close them on failure.
    // Credentials providers must be closed *after* the clients that reference them.
    val allClients: Seq[AutoCloseable] =
      Seq(sourceClient, streamsClient, targetClient, metrics) ++ closeableCredentials
    try
            ...
            try {
              ...
              result match {
                ...
                case LeaseRenewResult.Lost =>
                  log.warn(
                    s"Lost lease for shard $shardId during background renewal, " +
                      "scheduling removal from tracking"
                  )
                  worker.reportLostLease(shardId)
                case LeaseRenewResult.TransferRequested =>
                  log.info(
                    s"Lease transfer requested for shard $shardId during background renewal, " +
                      "scheduling removal from tracking"
                  )
                  worker.reportLostLease(shardId)
              }
