...
/** Thrown when BatchWriteItem retries are exhausted and unprocessed items remain.
  ...
  * @param successfullyFlushedCount
  *   number of items in sub-batches that were fully flushed before the failure
  */
class BatchWriteExhaustedException(
  message: String,
  val unprocessedCount: Int,
  val successfullyFlushedCount: Int = 0
...
object BatchWriter {
  ...
  /** Write a batch of stream change items to the target table. Items are grouped into
    ...
    *
    * On [[BatchWriteExhaustedException]], the exception's `successfullyFlushedCount` indicates how
    * many items (in prior sub-batches) were fully written before the failure. Callers can use this
    * to compute a partial rollback of per-shard sequence numbers.
    */
    ...
    if (msgs.isEmpty) {
      ...
      def flushBatch(): Unit =
        try {
          ...
          flushedItemCount += batchSize
        } catch {
          case e: BatchWriteExhaustedException =>
            throw new BatchWriteExhaustedException(
              e.getMessage,
              e.unprocessedCount,
              flushedItemCount
            )
      ...
      msgs.foreach { item =>
        ...
        // Use length-prefixed encoding to avoid ambiguity when values contain
        // separator characters. Format: "len:attr\0len:value\0..." where len is
        // the byte length of the following string.
        ...
        for (attr <- keyAttrNames) {
          ...
          if (value != null) {
            ...
            // Use canonical serialization for dedup key instead of toString(),
            // which is not guaranteed to be deterministic across SDK versions.
            // Key attributes are always S, N, or B in DynamoDB.
            val valueStr =
              if (value.s() != null) "S:" + value.s()
              else if (value.n() != null) "N:" + value.n()
              else if (value.b() != null)
                "B:" + java.util.Base64.getEncoder.encodeToString(
                  value.b().asByteArray()
                )
              else value.toString // fallback for unexpected types
            keyStringBuilder
              .append(renamedAttr.length)
              .append(':')
              .append(renamedAttr)
              .append('\u0000')
              .append(valueStr.length)
              .append(':')
              .append(valueStr)
              .append('\u0000')
          } else {
            log.warn(
              s"Skipping item with null key attribute '$attr' — " +
                "cannot build a valid key for deduplication or delete request"
            )
            keyComplete = false
          }
        }
        if (!keyComplete) {
          // Skip this item entirely — a partial key would produce a malformed
          // BatchWriteItem request that DynamoDB would reject.
        } else {
    ...
    try
      while (!unprocessed.isEmpty) {
        ...
        try {
          ...
          case e: DynamoDbException =>
            // Non-retryable DynamoDB error (e.g., ValidationException, AccessDeniedException).
            // Rethrow immediately — retrying would waste time on a permanent failure.
            throw e
        }
